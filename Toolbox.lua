...
task.spawn(function()
    ...
    print(_sys_name)
    print(_dev_name)
    print("Status: System Toolbox Complete")
    print(_load_msg)
    print("--------------------------------------------------")
end)
...
local function C_1e()
	...
	task.spawn(function()	
		...
		local Selection = game:GetService("Selection") -- Digunakan untuk deteksi objek terpilih (Decal Target)
		...
		-- Bindings untuk kustom fungsi StudioLite Explorer panel jika tersedia
		local ExplorerPanel = script.Parent.Parent:FindFirstChild("ExplorerPanel")
		local SetSelectionRemote = ExplorerPanel footprints ExplorerPanel:FindFirstChild("SetSelection")
		...
		-------------------------------------------------------------------------
		-- FUNGSI INSERT UTAMA (SINKRONISASI KATEGORI & LOGIKA IMAGE SELECTION)
		-------------------------------------------------------------------------
		local function InsertAsset(assetId, category, statusTarget)
			statusTarget.Text = "Working"
			local stringId = tostring(assetId)

			-- 1. Deteksi Kategori Secara Akurat Menggunakan info tipe asli Roblox API
			local success, info = pcall(function() return MarketplaceService:GetProductInfo(assetId) end)
			if success and info then
				local typeId = info.AssetTypeId
				if typeId == 1 or typeId == 13 or typeId == 3 or typeId == 14 then 
					category = "Decal"
				elseif typeId == 3 or typeId == 34 then 
					category = "Audio"
				else 
					category = "Model" 
				end
			else
				category = category or "Model" -- Fallback jika API gagal
			end
			...
			if category == "Audio" then
				...
				-- Taruh di dalam objek terpilih jika ada, jika tidak masukkan ke Workspace
				local selectedObjects = Selection:Get()
				if GetSelectionRemote then pcall(function() selectedObjects = GetSelectionRemote:Invoke() end) end
				
				if selectedObjects and #selectedObjects >= 1 then
					sound.Parent = selectedObjects[1]
				else
					sound.Parent = workspace
				end
				
				sound:Play()
				if SetSelectionRemote then pcall(function() SetSelectionRemote:Invoke({sound}) end) end
				statusTarget.Text = "Berhasil!"
				return
			end

			-------------------------------------------------------------------------
			-- LOGIKA REPLIKASI HIBRIDA (MODEL & DECAL VIA SERVER/CLIENT FALLBACK)
			-------------------------------------------------------------------------
			if LoadAssetRemote and LoadAssetRemote:IsA("RemoteFunction") then
				...
				if loadSuccess then
					...
					if serverFolder then
						...
						-- Bersihkan Baseplate bawaan dari template paket asset jika ada
						...
						for _, obj in pairs(assetClone:GetChildren()) do
							-- Replikasi folder system/services
									...
									for _, inner in pairs(obj:GetChildren()) do
										if inner.Name == "StarterPlayerScripts" or inner.Name == "StarterCharacterScripts" then
											for _, scr in pairs(inner:GetChildren()) do
												if not game.StarterPlayer[inner.Name]:FindFirstChild(scr.Name) then scr.Parent = game.StarterPlayer[inner.Name] end
											end
										else
							...
							elseif obj:IsA("PostEffect") or obj.ClassName == "Sky" then
								...
								-------------------------------------------------------------------------
								-- FITUR UTAMA: LOGIKA JIKA ASSET ADALAH DECAL / IMAGE
								-------------------------------------------------------------------------
								if category == "Decal" or obj:IsA("Decal") or obj:IsA("Texture") then
									local targetDecal = obj:Clone()
									local selectedObjects = Selection:Get()
									
									-- Dukungan pembacaan custom ExplorerPanel StudioLite
									if GetSelectionRemote then pcall(function() selectedObjects = GetSelectionRemote:Invoke() end) end
									
									if selectedObjects and #selectedObjects >= 1 and (selectedObjects[1]:IsA("BasePart") or selectedObjects[1]:IsA("MeshPart")) then
										targetDecal.Parent = selectedObjects[1] -- Masuk ke dalam part yang sedang dipilih
									else
										-- Jika tidak memilih part, buat part temporary otomatis di workspace sebagai penampung gambar
										local partPenampung = Instance.new("Part")
										partPenampung.Name = "Decal_Container_" .. stringId
										partPenampung.Size = Vector3.new(4, 4, 1)
										partPenampung.Position = workspace.Camera.CFrame.Position + (workspace.Camera.CFrame.LookVector * 10)
										partPenampung.Parent = workspace
										targetDecal.Parent = partPenampung
									end
									
									if SetSelectionRemote then pcall(function() SetSelectionRemote:Invoke({targetDecal}) end) end
								else
									-------------------------------------------------------------------------
									-- LOGIKA SPASIAL JIKA ASSET ADALAH MODEL 3D
									-------------------------------------------------------------------------
									...
									local finalSpawnedObj
									if isTemporary then
										finalSpawnedObj = targetModel:GetChildren()[1]:Clone()
										finalSpawnedObj.Parent = workspace
										if tempContainer then tempContainer:Destroy() end
									else
										targetModel.Parent = workspace
										finalSpawnedObj = targetModel
									end
									
									if SetSelectionRemote then pcall(function() SetSelectionRemote:Invoke({finalSpawnedObj}) end) end
								end
							end
						end
				...
				-------------------------------------------------------------------------
				-- EXECUTOR CLIENT FALLBACK (GETOBJECTS SYSTEM)
				-------------------------------------------------------------------------
				...
				if clientSuccess and clientObj then
					...
					if category == "Decal" or clone:IsA("Decal") or clone:IsA("Texture") then
						...
						if selectedObjects and #selectedObjects >= 1 and (selectedObjects[1]:IsA("BasePart") or selectedObjects[1]:IsA("MeshPart")) then
							...
							local partPenampung = Instance.new("Part")
							partPenampung.Name = "Decal_Container"
							partPenampung.Position = LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart.Position + Vector3.new(0, 2, 5) or Vector3.new(0,5,0)
							partPenampung.Parent = workspace
							clone.Parent = partPenampung
						end
					else
				...
				task.spawn(function()
					...
					if success and info then
						...
						elseif CurrentCategory == "Audio" then
							ImageAsset.Image = "rbxassetid://16327318049" -- Icon default Audio
						else
							ImageAsset.Image = "rbxthumb://type=Asset&id=" .. assetId .. "&w=150&h=150"
						end
		...
		-- Logika GetButton (Memasukkan ID Langsung Secara Otomatis Sesuai Tipe Aslinya)
		...
		-- Logika SaveButton Otomatis Menyortir & Menyimpan Masuk ke Kategori Masing-Masing
		SaveButton.MouseButton1Click:Connect(function()
			...
			if success and info then
				...
				-- Penyortiran Kategori Target secara Otomatis dan Akurat
				if typeId == 1 or typeId == 13 or typeId == 3 or typeId == 14 then 
					cat = "Decal"
				...
				if not isDup then
					...
					SaveData()
					SwitchTab(cat) -- Otomatis ganti tab ke kategori asset yang baru disimpan agar terlihat
