...
local function C_1e()
	...
	task.spawn(function()	
		...
		local readfile = readfile or io.writefile
		...
		-------------------------------------------------------------------------
		-- PEMANGGILAN OBJEK UI & SELECTION BINDINGS
		-------------------------------------------------------------------------
		local Gui = script.Parent	
		local MainPanel = Gui:WaitForChild("Panel")	
		local CloseButton = MainPanel:WaitForChild("CloseButton")	
		...
		-- Deteksi Sistem Seleksi Explorer Studio Lite (Dinamis & Akurat)
		local ExplorerPanel = Gui:FindFirstChild("ExplorerPanel") or (Gui.Parent and Gui.Parent:FindFirstChild("ExplorerPanel", true))
		...
		-------------------------------------------------------------------------
		-- FUNGSI INSERT UTAMA (FIXED SELECTION FOR DECALS / IMAGES)
		-------------------------------------------------------------------------
		local function InsertAsset(assetId, category, statusTarget)
			...
			-----------------------------------------------------------------
			-- LOGIKA KHUSUS DECAL (MENEMPEL PADA PART YANG DIPILIH)
			-----------------------------------------------------------------
			if category == "Decal" then
				-- 1. Ambil target part yang sedang dipilih melalui ExplorerPanel Studio Lite
				local selection = GetSelection and GetSelection:Invoke()
				local targetPart = nil
				if selection and typeof(selection) == "table" and #selection >= 1 then
					if selection[1]:IsA("BasePart") then
						targetPart = selection[1]
					end
				end

				-- 2. Fallback: Menggunakan kursor mouse apabila tidak ada item terpilih di Explorer
				if not targetPart then
					local Mouse = LocalPlayer:GetMouse()
					if Mouse.Target and Mouse.Target:IsA("BasePart") and Mouse.Target.Name ~= "Baseplate" then
				...
				if LoadAssetRemote and LoadAssetRemote:IsA("RemoteFunction") then
					...
					if loadSuccess then
						...
						if serverFolder then
							...
							local actualDecal = assetClone:IsA("Decal") and assetClone or assetClone:FindFirstChildOfClass("Decal")
							
							if actualDecal then
								if targetPart then
									actualDecal.Parent = targetPart
									statusTarget.Text = "Applied!"
								else
									actualDecal.Parent = workspace
									statusTarget.Text = "Berhasil!"
								end
								if SetSelection then pcall(function() SetSelection:Invoke({actualDecal}) end) end
							else
								if targetPart then
									assetClone.Parent = targetPart
									statusTarget.Text = "Applied!"
								else
									assetClone.Parent = workspace
									statusTarget.Text = "Berhasil!"
								end
								if SetSelection then pcall(function() SetSelection:Invoke({assetClone}) end) end
							end
					...
					-- Executor Client Fallback (Jika Remote Server tidak merespon)
					...
					if clientSuccess and clientObj then
						...
						local actualDecal = clone:IsA("Decal") and clone or clone:FindFirstChildOfClass("Decal")
						...
						if SetSelection then pcall(function() SetSelection:Invoke({actualDecal or clone}) end) end
					else
			...
			-----------------------------------------------------------------
			-- LOGIKA KHUSUS AUDIO (Langsung ke Workspace)
			-----------------------------------------------------------------
			...
			-----------------------------------------------------------------
			-- LOGIKA MODEL & MESH (Studio Lite / Executor Fallback)
			-----------------------------------------------------------------
			if LoadAssetRemote and LoadAssetRemote:IsA("RemoteFunction") then
				local loadSuccess = false
				pcall(function()
