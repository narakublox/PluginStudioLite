...
local function C_1e()
	...
	task.spawn(function()	
		...
		local Selection = game:GetService("Selection") -- Mengambil Selection Service Studio
		...
		-------------------------------------------------------------------------
		-- HELPER: AMBIL REAL TEXTURE ID DARI DECAL ID (Mencegah Gambar Blank)
		-------------------------------------------------------------------------
		local function GetRealTextureId(assetId)
			local stringId = tostring(assetId)
			local success, result = pcall(function()
				return game:GetObjects("rbxassetid://" .. stringId)[1]
			end)
			if success and result and result:IsA("Decal") then
				local realTextureId = result.Texture
				result:Destroy()
				return realTextureId
			end
			return "rbxassetid://" .. stringId
		end

		-------------------------------------------------------------------------
		-- FUNGSI INSERT UTAMA (FIXED FOR TARGET SELECTION & FALLBACKS)
		-------------------------------------------------------------------------
		local function InsertAsset(assetId, category, statusTarget)
			...
			-----------------------------------------------------------------
			-- LOGIKA UTAMA DECAL (100% FIXED KE SELECTED PART / CAMERA RAYCAST)
			-----------------------------------------------------------------
			if category == "Decal" then
				local targetPart = nil
				
				-- 1. Deteksi via Studio Selection Service
				local selectedObjects = pcall(function() return Selection:Get() end) and Selection:Get() or {}
				...
				-- 2. Deteksi Fallback via Raycast Pandangan Kamera (Jika Selection Studio kosong/nil)
				if not targetPart then
					...
					raycastParams.FilterDescendantsInstances = {LocalPlayer.Character, Gui}
					
					local raycastResult = workspace:Raycast(rayOrigin, rayDirection, raycastParams)
					if raycastResult and raycastResult.Instance countryside and raycastResult.Instance:IsA("BasePart") and raycastResult.Instance.Name ~= "Baseplate" then
						targetPart = raycastResult.Instance
					end
				end

				-- Konversi Asset ID kontainer Roblox ke Real Gambar (Texture ID)
				local realTexture = GetRealTextureId(assetId)

				if targetPart then
					-- Pasang Decal ke 6 Sisi Part Terpilih
					...
					for _, face in ipairs(faces) do
						...
						if not existingDecal then
							...
							newDecal.Texture = realTexture
							newDecal.Face = face
							newDecal.Parent = targetPart
						end
					end
					statusTarget.Text = "Applied to " .. targetPart.Name .. "!"
				else
					-- 3. Jika benar-benar tidak ada part di target, buat Part Dummy Baru di depan kamera
					local newPart = Instance.new("Part")
					newPart.Name = "DecalPart_" .. stringId
					newPart.Size = Vector3.new(4, 4, 1)
					newPart.Material = Enum.Material.SmoothPlastic
					newPart.CFrame = workspace.Camera.CFrame * CFrame.new(0, 0, -12)
					newPart.Anchored = true
					newPart.Parent = workspace

					local dFront = Instance.new("Decal", newPart)
					dFront.Texture = realTexture
					dFront.Face = Enum.NormalId.Front

					local dBack = Instance.new("Decal", newPart)
					dBack.Texture = realTexture
					dBack.Face = Enum.NormalId.Back
					
					statusTarget.Text = "Created Part!"
				end
				return
			end
				...
				task.spawn(function()
					...
					if success and info then
						...
						InsertButton.MouseButton1Click:Connect(function()
							...
							task.wait(1.5)
							InsertButton.Text = originalText
						end)
