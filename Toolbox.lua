-- TOOLBOX STUDIO LITE ROBLOX EXECUTOR 
-- By NARAKU BETA
local LMG2L = {};
...
local function C_1e()
	...
	task.spawn(function()	
		...
		local function GetCategoryFromAssetType(assetTypeId)
			-- 1 = Image, 2 = Texture, 13 = Decal, 14 = ImageButton (Dikelompokkan ke Decal)
			if assetTypeId == 13 or assetTypeId == 1 or assetTypeId == 2 or assetTypeId == 14 then
				return "Decal"
			-- 3 = Audio, 34 = Sound Effect
		...
		local function InsertAsset(assetId, category, statusTarget)
			...
			-- Ambil informasi riil dari Marketplace jika kategori kosong/belum ditentukan
			...
			if successInfo and info then
				...
				if not category then category = "Model" end -- Fallback
			end

			-------------------------------------------------------------------------
			-- PERBAIKAN LOGIKA 1: AUDIO HANDLING
			-------------------------------------------------------------------------
			...
			-------------------------------------------------------------------------
			-- PERBAIKAN LOGIKA 2: DECAL / IMAGE HANDLING
			-------------------------------------------------------------------------
			if category == "Decal" then
				-- Membuat objek Decal langsung di Workspace agar bisa langsung digunakan/diambil
				...
				-- Coba seleksi otomatis jika script eksekutor mendukung tool seleksi
				statusTarget.Text = "Berhasil!"
				return
			end

			-------------------------------------------------------------------------
			-- PERBAIKAN LOGIKA 3: MODEL HANDLING VIA REMOTE / FALLBACK
			-------------------------------------------------------------------------
			if LoadAssetRemote and LoadAssetRemote:IsA("RemoteFunction") then
				...
				if loadSuccess then
					...
					if serverFolder then
						...
						-- Jika folder kosong / isinya langsung object tanpa sub-folder Workspace
						local children = assetClone:GetChildren()
						if #children == 0 then
							-- Fallback jika isi kosong tetapi remote berhasil
						...
						-- Jika folder PlayerGui tidak muncul, gunakan client fallback langsung
				...
				-- Executor Client Fallback (Jika game dijalankan di Luau Executor biasa)
				...
				task.spawn(function()
					...
					if success and info then
						...
						elseif CurrentCategory == "Audio" then
							ImageAsset.Image = "rbxassetid://16327318049" -- Default Audio Icon Template Anda
						else
							ImageAsset.Image = "rbxthumb://type=Asset&id=" .. assetId .. "&w=150&h=150"
						end
		...
		-------------------------------------------------------------------------
		-- FIX PENYIMPANAN DATA SECARA SINKRON DAN AKURAT SESUAI KATEGORI ASLI
		-------------------------------------------------------------------------
		SaveButton.MouseButton1Click:Connect(function()
			...
			if success and info then
				-- Menentukan kategori asli berdasarkan AssetTypeId dari database Roblox resmi
				local cat = GetCategoryFromAssetType(info.AssetTypeId)

				local isDup = false
				for _, id in ipairs(SavedAssets[cat]) do 
					if id == cleanId then isDup = true break end 
				end

				if not isDup then
					table.insert(SavedAssets[cat], cleanId)
					SaveData()
					SwitchTab(cat) -- Otomatis beralih ke tab kategori yang sesuai saat berhasil save
